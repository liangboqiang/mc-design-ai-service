---
id: skill/wiki/page_diagnosis
kind: Skill
status: published
maturity: projectable
lens: lens.skill
source_refs:
  - legacy.wiki.compat
tags:
  - migrated
---

# 技能：页面诊断技能

摘要：检查页面中文元结构、自然链接、锁定禁用状态、依赖文件和局部关系。

## 基本信息

- 实体类型：技能
- 实体名称：页面诊断技能
- 唯一标识：skill/wiki/page_diagnosis
- 当前状态：可用
- 锁定状态：未锁定
- 禁用状态：未禁用
- 最近更新：2026-04-25

## 元词条

- 关键词：页面诊断技能、Wiki Agent、中文 Wiki、真相治理
- 别名：页面诊断技能、skill/wiki/page_diagnosis
- 风险等级：中
- 作用范围：Wiki 页面、真实文件、草稿、版本和图谱
- 局部关系：属于 [[Wiki 治理根技能|skill/wiki/root]]
- 依赖文件：相关工具页面和元结构页面
- 更新策略：差异化更新

## 技能目标

检查页面中文元结构、自然链接、锁定禁用状态、依赖文件和局部关系。

## 使用工具

- [[检查页面元结构|tool/wiki/check_page_schema]]
- [[解析页面链接|tool/wiki/resolve_page_links]]
- [[页面依赖状态检查|tool/wiki/page_file_status]]
- [[页面局部关系|tool/wiki/page_scope_relations]]
- [[诊断页面问题|tool/wiki/diagnose_page]]

## 执行流程

- 读取页面诊断结果。
- 识别缺失字段、断链、歧义链接和依赖变化。
- 给出可执行修复建议。

## 下级技能

- 待补充

## 安全边界

- 不允许生成 Runtime YAML。
- 不允许直接修改真相文件。
- 写操作必须先生成草稿或候选结果。
- 锁定页面和禁用页面禁止自动更新和修复。
- 禁用页面默认不参与知识图谱生成。
- 发布必须由用户明确授权。

## 关联页面

- [[Wiki 治理工具箱|tool/wiki]]
- [[Wiki 只读工具箱|tool/external/wiki]]
- [[通用页面元结构|wiki/schema/common]]

## 版本信息

- 当前版本：v1
- 最近发布：待发布
- 最近修改人：系统
- 版本来源：Git
