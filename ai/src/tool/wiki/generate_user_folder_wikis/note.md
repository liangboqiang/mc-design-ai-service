---
id: tool/wiki/generate_user_folder_wikis
kind: Tool
status: published
...
摘要：面向用户文件目录自底向上生成 wiki.md 候选页面；下层目录已有 wiki.md 时只引用该页面，不重复读取全部下层文件。

## 基本信息

- 实体类型：工具
- 实体名称：用户文件夹建页
- 唯一标识：wiki_app.generate_user_folder_wikis
- 当前状态：可用
- 锁定状态：未锁定
- 禁用状态：未禁用
- 最近更新：2026-04-25
...
- 关键词：用户文件夹建页、Wiki Agent、页面治理、知识图谱
- 别名：用户文件夹建页、wiki_app.generate_user_folder_wikis
- 风险等级：中
...
面向用户文件目录自底向上生成 wiki.md 候选页面；下层目录已有 wiki.md 时只引用该页面，不重复读取全部下层文件。
