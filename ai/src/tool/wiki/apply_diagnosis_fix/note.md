---
id: tool/wiki/apply_diagnosis_fix
kind: Tool
status: published
...
摘要：根据诊断建议执行指定修复动作，所有修复都生成草稿或候选结果，不绕过锁定、禁用和发布机制。

## 基本信息

- 实体类型：工具
- 实体名称：执行诊断修复
- 唯一标识：wiki_app.apply_diagnosis_fix
- 当前状态：可用
- 锁定状态：未锁定
- 禁用状态：未禁用
- 最近更新：2026-04-25
...
- 关键词：执行诊断修复、Wiki Agent、页面治理、知识图谱
- 别名：执行诊断修复、wiki_app.apply_diagnosis_fix
- 风险等级：中
...
根据诊断建议执行指定修复动作，所有修复都生成草稿或候选结果，不绕过锁定、禁用和发布机制。
